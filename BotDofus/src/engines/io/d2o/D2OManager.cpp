...
void D2OManager::init(const QString &D2OFolderPath, const QString &I18nFilePath)
{
    ...
    if (!QDir(D2OFolderPath).exists())
        qDebug()<<"ERROR - D2OManager - Cannot open the D2Os folder :"<<D2OFolderPath;

    if (!QFile(I18nFilePath).exists())
        qDebug()<<"ERROR - D2OManager - Cannot open the I18n file :"<<I18nFilePath;
    ...
    qDebug()<<"[D2OManager] Initialized ! ("<<t.elapsed()<<"ms)";
}
...
QSharedPointer<AbstractGameData> D2OManager::getObject(GameDataTypeEnum type, int index)
{
    ...
    if (!m_isInit)
        qDebug()<<"ERROR - D2OManager - Is not initialized!";
...
void D2OManager::loadFile(GameDataTypeEnum searchedType)
{
    ...
    if(!foundFile)
        qDebug()<<"ERROR - D2OManager - Don't know the enum"<<(int)searchedType;
}
