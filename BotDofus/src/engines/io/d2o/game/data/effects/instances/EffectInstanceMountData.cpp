...
QList<EffectInstanceIntegerData> EffectInstanceMountData::getEffects() const
{
  return m_effects;
}
  ...
  foreach (D2OField *field, fields)
  {
    ...
    else if(field->getName() == "effects")
    {
      foreach(const QByteArray &data, readVector(field->getValue()))
          m_effects << *qSharedPointerCast<EffectInstanceIntegerData>(readObject(data, field)).data();
    }
