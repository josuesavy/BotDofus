...
QList<PopupButtonData> PopupInformationData::getButtons() const
{
  return m_buttons;
}
...
void PopupInformationData::loadData(const QList<D2OField*> &fields, I18nFile *I18n)
{
  ...
  foreach (D2OField *field, fields)
  {
    ...
    else if(field->getName() == "buttons")
    {
      foreach(const QByteArray &data, readVector(field->getValue()))
          m_buttons << *qSharedPointerCast<PopupButtonData>(readObject(data, field)).data();
    }
