...
void HumanOptionFollowers::serializeAs_HumanOptionFollowers(Writer *output)
{
  ...
  {
    qSharedPointerCast<IndexedEntityLook>(this->followingCharactersLook[_i1])->serializeAs_IndexedEntityLook(output);
  }
}
...
{
  QSharedPointer<IndexedEntityLook> _item1 ;
  HumanOption::deserialize(input);
  uint _followingCharactersLookLen = input->readUShort();
  for(uint _i1 = 0; _i1 < _followingCharactersLookLen; _i1++)
  {
    _item1 = QSharedPointer<IndexedEntityLook>(new IndexedEntityLook() );
...
{
  QSharedPointer<IndexedEntityLook> _item = QSharedPointer<IndexedEntityLook>(new IndexedEntityLook() );
