...
bool GameActionsFightFrame::processMessage(const MessageInfos &data, SocketIO *sender)
{
    ...
    case MessageEnum::GAMEACTIONFIGHTDEATHMESSAGE:
    {
        ...
        m_botData[sender].fightData.fighters[message.targetId].stats[(uint)StatIds::LIFE_POINTS].base = 0;
    }
        break;
    ...
    case MessageEnum::GAMEACTIONFIGHTDISPELLABLEEFFECTMESSAGE:
    {
        ...
        else if(message.effect->getTypes().contains(ClassEnum::FIGHTTEMPORARYBOOSTEFFECT))
        {
            ...
            if (message.actionId == 168)
                m_botData[sender].fightData.fighters[message.sourceId].stats[(uint)StatIds::ACTION_POINTS].total -= effect->delta;
    ...
    case MessageEnum::GAMEACTIONFIGHTLIFEANDSHIELDPOINTSLOSTMESSAGE:
    {
        ...
        m_botData[sender].fightData.fighters[message.targetId].stats[(uint)StatIds::LIFE_POINTS].base -= message.loss;
        m_botData[sender].fightData.fighters[message.targetId].stats[(uint)StatIds::MAX_LIFE].base -= message.permanentDamages;

        if(m_botData[sender].fightData.botFightData.botId == message.targetId)
        {
            m_botData[sender].playerData.stats[(uint)StatIds::LIFE_POINTS].base -= message.loss;
            m_botData[sender].playerData.stats[(uint)StatIds::MAX_LIFE].base -= message.permanentDamages;
        }
    }
        break;
    ...
    case MessageEnum::GAMEACTIONFIGHTLIFEPOINTSGAINMESSAGE:
    {
        ...
        m_botData[sender].fightData.fighters[message.targetId].stats[(uint)StatIds::LIFE_POINTS].base += message.delta;

        if(m_botData[sender].fightData.botFightData.botId == message.targetId)
            m_botData[sender].playerData.stats[(uint)StatIds::LIFE_POINTS].base += message.delta;
    }
        break;
    ...
    case MessageEnum::GAMEACTIONFIGHTPOINTSVARIATIONMESSAGE:
    {
        ...
        case 102:
        case 120:
            m_botData[sender].fightData.fighters[message.sourceId].stats[(uint)StatIds::ACTION_POINTS].total += message.delta;
            break;
        case 78:
        case 127:
        case 129:
            m_botData[sender].fightData.fighters[message.sourceId].stats[(uint)StatIds::MOVEMENT_POINTS].total += message.delta;
            break;
        }
    }
        break;
