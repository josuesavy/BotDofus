...
void FightCommonInformations::serializeAs_FightCommonInformations(Writer *output)
{
  ...
  {
    output->writeShort((short)qSharedPointerCast<FightTeamInformations>(this->fightTeams[_i3])->getTypes().last());
    qSharedPointerCast<FightTeamInformations>(this->fightTeams[_i3])->serialize(output);
  }
...
{
  uint _id3 = 0;
  QSharedPointer<FightTeamInformations> _item3 ;
  uint _val4 = 0;
  FightOptionsInformations _item5 ;
  this->_fightIdFunc(input);
  this->_fightTypeFunc(input);
  uint _fightTeamsLen = input->readUShort();
  for(uint _i3 = 0; _i3 < _fightTeamsLen; _i3++)
  {
    _id3 = input->readUShort();
    _item3 = qSharedPointerCast<FightTeamInformations>(ClassManagerSingleton::get()->getClass(_id3));
    _item3->deserialize(input);
    this->fightTeams.append(_item3);
  }
  uint _fightTeamsPositionsLen = input->readUShort();
  ...
  uint _fightTeamsOptionsLen = input->readUShort();
  for(uint _i5 = 0; _i5 < _fightTeamsOptionsLen; _i5++)
  {
    _item5 = FightOptionsInformations();
...
void FightCommonInformations::_fightTeamsFunc(Reader *input)
{
  ...
  QSharedPointer<FightTeamInformations> _item = qSharedPointerCast<FightTeamInformations>(ClassManagerSingleton::get()->getClass(_id));
...
{
  FightOptionsInformations _item = FightOptionsInformations();
  _item.deserialize(input);
