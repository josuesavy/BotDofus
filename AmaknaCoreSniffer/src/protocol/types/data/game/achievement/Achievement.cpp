...
void Achievement::deserializeAs_Achievement(Reader *input)
{
  ...
  this->_idFunc(input);
  uint _finishedObjectiveLen = input->readUShort();
  for(uint _i2 = 0; _i2 < _finishedObjectiveLen; _i2++)
  {
    _item2 = AchievementObjective();
    _item2.deserialize(input);
    this->finishedObjective.append(_item2);
  }
  uint _startedObjectivesLen = input->readUShort();
  for(uint _i3 = 0; _i3 < _startedObjectivesLen; _i3++)
  {
    _item3 = AchievementStartedObjective();
...
{
  AchievementObjective _item = AchievementObjective();
...
{
  AchievementStartedObjective _item = AchievementStartedObjective();
  _item.deserialize(input);
