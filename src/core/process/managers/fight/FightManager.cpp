...
SpellInabilityReason FightManager::canCastSpell(SocketIO *sender, int spellID)
{
    ...
    qDebug() << "SpellData::getStatesCriterion():" << spellLevelsData->getStatesCriterion();
...
//    }
    ...
    QString stateCriterion = spellLevelsData->getStatesCriterion();
    if (stateCriterion.isNull() || stateCriterion.isEmpty())
    {
        return SpellInabilityReason::FORBIDDEN_STATE;
    }
    QList<int> values;
    if (stateCriterion.contains("HS"))
    {
    ...
    {
        QSharedPointer<SpellStateData> spellStateData = qSharedPointerCast<SpellStateData>(D2OManagerSingleton::get()->getObject(GameDataTypeEnum::SPELLSTATES, value));
        if (spellStateData->getPreventsSpellCast())
        {
            if (m_botData[sender].fightData.botFightData.durationByEffect.contains(value))
            {
                return SpellInabilityReason::FORBIDDEN_STATE;
            }
        }
