...
void InventoryWeightMessage::serializeAs_InventoryWeightMessage(Writer *output)
{
  ...
  {
    qDebug()<<"ERREUR - InventoryWeightMessage -"<<"Forbidden value (" << this->shopWeight << ") on element shopWeight.";
  }
  output->writeVarInt((int)this->shopWeight);
  if(this->weightMax < 0)
  {
...
void InventoryWeightMessage::deserializeAsyncAs_InventoryWeightMessage(FuncTree tree)
{
  ...
  tree.addChild(std::bind(&InventoryWeightMessage::_shopWeightFunc, this, std::placeholders::_1));
...
void InventoryWeightMessage::_shopWeightFunc(Reader *input)
{
  this->shopWeight = input->readVarUhInt();
  if(this->shopWeight < 0)
  {
    qDebug()<<"ERREUR - InventoryWeightMessage -"<<"Forbidden value (" << this->shopWeight << ") on element of InventoryWeightMessage.shopWeight.";
  }
}
