...
bool ItemData::getIsColorable() const
{
...
double ItemData::getRecyclingNuggets() const
{
  return m_recyclingNuggets;
}

QList<uint> ItemData::getFavoriteRecyclingSubareas() const
{
  return m_favoriteRecyclingSubareas;
}
...
void ItemData::loadData(const QList<D2OField*> &fields, I18nFile *I18n)
{
  ...
  foreach (D2OField *field, fields)
  {
    ...
    else if(field->getName() == "isColorable")
        m_isColorable = readBool(field->getValue());
    ...
    else if(field->getName() == "recyclingNuggets")
        m_recyclingNuggets = readDouble(field->getValue());
    
    else if(field->getName() == "favoriteRecyclingSubareas")
    {
      foreach(const QByteArray &data, readVector(field->getValue()))
          m_favoriteRecyclingSubareas << readUInt(data);
    }
