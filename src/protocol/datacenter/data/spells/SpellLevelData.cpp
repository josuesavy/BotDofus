...
bool SpellLevelData::getNeedVisibleEntity() const
{
  return m_needVisibleEntity;
}

bool SpellLevelData::getNeedCellWithoutPortal() const
{
  return m_needCellWithoutPortal;
}

bool SpellLevelData::getPortalProjectionForbidden() const
{
  return m_portalProjectionForbidden;
}
...
QList<EffectZoneData> SpellLevelData::getPreviewZones() const
{
  return m_previewZones;
}
...
void SpellLevelData::loadData(const QList<D2OField*> &fields, I18nFile *I18n)
{
  ...
  foreach (D2OField *field, fields)
  {
    ...
    else if(field->getName() == "needVisibleEntity")
        m_needVisibleEntity = readBool(field->getValue());
    
    else if(field->getName() == "needCellWithoutPortal")
        m_needCellWithoutPortal = readBool(field->getValue());
    
    else if(field->getName() == "portalProjectionForbidden")
        m_portalProjectionForbidden = readBool(field->getValue());
    ...
    else if(field->getName() == "previewZones")
    {
      foreach(const QByteArray &data, readVector(field->getValue()))
          m_previewZones << *qSharedPointerCast<EffectZoneData>(readObject(data, field)).data();
    }
