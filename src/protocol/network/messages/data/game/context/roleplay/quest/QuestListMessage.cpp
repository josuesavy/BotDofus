...
void QuestListMessage::deserializeAs_QuestListMessage(Reader *input)
{
  ...
  uint _val4 = 0;
  uint _finishedQuestsIdsLen = uint(input->readUShort());
  ...
  uint _finishedQuestsCountsLen = uint(input->readUShort());
  ...
  uint _activeQuestsLen = uint(input->readUShort());
  ...
  uint _reinitDoneQuestsIdsLen = uint(input->readUShort());
