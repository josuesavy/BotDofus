...
void GuildSummaryRequestMessage::serializeAs_GuildSummaryRequestMessage(Writer *output)
{
  ...
  for(uint _i4 = 0; _i4 < this->criterionFilter.size(); _i4++)
  {
    if(this->criterionFilter[_i4] < 0)
    {
      qDebug()<<"ERREUR - GuildSummaryRequestMessage -"<<"Forbidden value (" << this->criterionFilter[_i4] << ") on element 4 (starting at 1) of criterionFilter.";
    }
    output->writeVarInt((int)this->criterionFilter[_i4]);
  }
  output->writeShort((short)this->languagesFilter.size());
  for(uint _i5 = 0; _i5 < this->languagesFilter.size(); _i5++)
  {
    if(this->languagesFilter[_i5] < 0)
    {
      qDebug()<<"ERREUR - GuildSummaryRequestMessage -"<<"Forbidden value (" << this->languagesFilter[_i5] << ") on element 5 (starting at 1) of languagesFilter.";
    }
    output->writeVarInt((int)this->languagesFilter[_i5]);
  }
  output->writeShort((short)this->recruitmentTypeFilter.size());
  for(uint _i6 = 0; _i6 < this->recruitmentTypeFilter.size(); _i6++)
  {
...
void GuildSummaryRequestMessage::deserializeAs_GuildSummaryRequestMessage(Reader *input)
{
  ...
  for(uint _i4 = 0; _i4 < _criterionFilterLen; _i4++)
  {
    _val4 = input->readVarUhInt();
    if(_val4 < 0)
    {
      qDebug()<<"ERREUR - GuildSummaryRequestMessage -"<<"Forbidden value (" << _val4 << ") on elements of criterionFilter.";
    }
    this->criterionFilter.append(_val4);
  }
  uint _languagesFilterLen = uint(input->readUShort());
  for(uint _i5 = 0; _i5 < _languagesFilterLen; _i5++)
  {
    _val5 = input->readVarUhInt();
    if(_val5 < 0)
    {
      qDebug()<<"ERREUR - GuildSummaryRequestMessage -"<<"Forbidden value (" << _val5 << ") on elements of languagesFilter.";
    }
    this->languagesFilter.append(_val5);
  }
  uint _recruitmentTypeFilterLen = uint(input->readUShort());
  for(uint _i6 = 0; _i6 < _recruitmentTypeFilterLen; _i6++)
  {
    _val6 = uint(input->readByte());
    if(_val6 < 0)
    {
      qDebug()<<"ERREUR - GuildSummaryRequestMessage -"<<"Forbidden value (" << _val6 << ") on elements of recruitmentTypeFilter.";
    }
    this->recruitmentTypeFilter.append(_val6);
  }
