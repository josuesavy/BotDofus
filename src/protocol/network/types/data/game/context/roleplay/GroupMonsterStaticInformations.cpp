...
{
   _item2 ;
  this->mainCreatureLightInfos = new com.ankamagames.dofus.network.types.game.context.roleplay.MonsterInGroupLightInformations();
  this->mainCreatureLightInfos.deserialize(input);
  uint _underlingsLen = uint(input->readUShort());
  for(uint _i2 = 0; _i2 < _underlingsLen; _i2++)
  {
    _item2 = new com.ankamagames.dofus.network.types.game.context.roleplay.MonsterInGroupInformations();
...
{
   _item = new com.ankamagames.dofus.network.types.game.context.roleplay.MonsterInGroupInformations();
