...
void AlternativeMonstersInGroupLightInformations::serializeAs_AlternativeMonstersInGroupLightInformations(Writer *output)
{
  ...
  {
    (this->monsters[_i2] as com.ankamagames.dofus.network.types.game.context.roleplay.MonsterInGroupLightInformations).serializeAs_MonsterInGroupLightInformations(output);
  }
}
...
{
  com.ankamagames.dofus.network.types.game.context.roleplay.MonsterInGroupLightInformations _item2 ;
  this->_playerCountFunc(input);
  uint _monstersLen = uint(input->readUShort());
  for(uint _i2 = 0; _i2 < _monstersLen; _i2++)
  {
    _item2 = new com.ankamagames.dofus.network.types.game.context.roleplay.MonsterInGroupLightInformations();
...
{
   _item = new com.ankamagames.dofus.network.types.game.context.roleplay.MonsterInGroupLightInformations();
