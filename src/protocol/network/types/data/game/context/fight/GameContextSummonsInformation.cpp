...
{
  output->writeShort((short)this->spawnInformation.getTypeId());
...
void GameContextSummonsInformation::deserializeAs_GameContextSummonsInformation(Reader *input)
{
  ...
  this->spawnInformation = qSharedPointerCast<>(ClassManagerSingleton::get()->getClass(_id1));
  this->spawnInformation.deserialize(input);
  this->_waveFunc(input);
  this->look = EntityLook();
  this->look.deserialize(input);
  uint _id4 = uint(input->readUShort());
  this->stats = qSharedPointerCast<>(ClassManagerSingleton::get()->getClass(_id4));
  ...
  for(uint _i5 = 0; _i5 < _summonsLen; _i5++)
  {
    ...
    _item5 = qSharedPointerCast<>(ClassManagerSingleton::get()->getClass(_id5));
...
void GameContextSummonsInformation::_spawnInformationtreeFunc(Reader *input)
{
  ...
  this->spawnInformation = qSharedPointerCast<>(ClassManagerSingleton::get()->getClass(_id));
...
{
  uint _id = uint(input->readUShort());
  this->stats = qSharedPointerCast<>(ClassManagerSingleton::get()->getClass(_id));
