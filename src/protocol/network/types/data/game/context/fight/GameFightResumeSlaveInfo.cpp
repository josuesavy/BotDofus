...
void GameFightResumeSlaveInfo::serializeAs_GameFightResumeSlaveInfo(Writer *output)
{
  ...
  {
    (this->spellCooldowns[_i2] as com.ankamagames.dofus.network.types.game.context.fight.GameFightSpellCooldown).serializeAs_GameFightSpellCooldown(output);
  }
...
{
  com.ankamagames.dofus.network.types.game.context.fight.GameFightSpellCooldown _item2 ;
  this->_slaveIdFunc(input);
  uint _spellCooldownsLen = uint(input->readUShort());
  for(uint _i2 = 0; _i2 < _spellCooldownsLen; _i2++)
  {
    _item2 = new com.ankamagames.dofus.network.types.game.context.fight.GameFightSpellCooldown();
...
{
   _item = new com.ankamagames.dofus.network.types.game.context.fight.GameFightSpellCooldown();
