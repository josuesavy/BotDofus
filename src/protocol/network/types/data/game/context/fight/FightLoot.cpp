...
void FightLoot::serializeAs_FightLoot(Writer *output)
{
  ...
  {
    (this->objects[_i1] as com.ankamagames.dofus.network.types.game.context.fight.FightLootObject).serializeAs_FightLootObject(output);
  }
...
{
  com.ankamagames.dofus.network.types.game.context.fight.FightLootObject _item1 ;
  uint _objectsLen = uint(input->readUShort());
  for(uint _i1 = 0; _i1 < _objectsLen; _i1++)
  {
    _item1 = new com.ankamagames.dofus.network.types.game.context.fight.FightLootObject();
...
{
   _item = new com.ankamagames.dofus.network.types.game.context.fight.FightLootObject();
