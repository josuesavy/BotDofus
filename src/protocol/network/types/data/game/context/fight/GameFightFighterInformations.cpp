...
void GameFightFighterInformations::deserializeAs_GameFightFighterInformations(Reader *input)
{
  ...
  this->spawnInfo = new com.ankamagames.dofus.network.types.game.context.fight.GameContextBasicSpawnInformation();
  this->spawnInfo.deserialize(input);
  this->_waveFunc(input);
  uint _id3 = uint(input->readUShort());
  this->stats = qSharedPointerCast<>(ClassManagerSingleton::get()->getClass(_id3));
  this->stats.deserialize(input);
  uint _previousPositionsLen = uint(input->readUShort());
