...
void SocialFight::serializeAs_SocialFight(Writer *output)
{
  ...
  {
    qSharedPointerCast<CharacterMinimalPlusLookInformations>(this->attackers[_i2])->serializeAs_CharacterMinimalPlusLookInformations(output);
  }
  output->writeShort((short)this->defenders.size());
  for(uint _i3 = 0; _i3 < this->defenders.size(); _i3++)
  {
    qSharedPointerCast<CharacterMinimalPlusLookInformations>(this->defenders[_i3])->serializeAs_CharacterMinimalPlusLookInformations(output);
  }
...
void SocialFight::deserializeAs_SocialFight(Reader *input)
{
  ...
  {
    _item3 = QSharedPointer<CharacterMinimalPlusLookInformations>(new CharacterMinimalPlusLookInformations() );
