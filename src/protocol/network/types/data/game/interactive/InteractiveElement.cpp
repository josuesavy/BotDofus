...
void InteractiveElement::serializeAs_InteractiveElement(Writer *output)
{
  ...
  {
    output->writeShort((short)(this->enabledSkills[_i3]).getTypeId());
  ...
  {
    output->writeShort((short)(this->disabledSkills[_i4]).getTypeId());
...
void InteractiveElement::deserializeAs_InteractiveElement(Reader *input)
{
  ...
  uint _enabledSkillsLen = uint(input->readUShort());
  ...
  uint _disabledSkillsLen = uint(input->readUShort());
  for(uint _i4 = 0; _i4 < _disabledSkillsLen; _i4++)
  {
    _id4 = uint(input->readUShort());
    _item4 = qSharedPointerCast<>(ClassManagerSingleton::get()->getClass(_id4));
