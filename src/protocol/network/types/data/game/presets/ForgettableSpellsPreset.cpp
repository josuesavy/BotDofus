...
void ForgettableSpellsPreset::deserializeAs_ForgettableSpellsPreset(Reader *input)
{
  ...
  this->baseSpellsPreset = new com.ankamagames.dofus.network.types.game.presets.SpellsPreset();
  this->baseSpellsPreset.deserialize(input);
  uint _forgettableSpellsLen = uint(input->readUShort());
  for(uint _i2 = 0; _i2 < _forgettableSpellsLen; _i2++)
  {
    _item2 = new com.ankamagames.dofus.network.types.game.presets.SpellForPreset();
